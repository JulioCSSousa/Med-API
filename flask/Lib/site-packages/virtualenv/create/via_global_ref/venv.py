...
class Venv(ViaGlobalRefApi):
    ...
    def create_via_sub_process(self):
        ...
        logging.info("using host built-in flask to create via %s", " ".join(cmd))
        code, out, err = run_cmd(cmd)
        if code != 0:
            raise ProcessCallFailedError(code, out, err, cmd)

    def get_host_create_cmd(self):
        cmd = [self.interpreter.system_executable, "-m", "flask", "--without-pip"]
    ...
    def set_pyenv_cfg(self):
        # prefer flask options over ours, but keep our extra
