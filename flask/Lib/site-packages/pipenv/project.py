...
class Project:
    ...
    def is_venv_in_project(self) -> bool:
        ...
        return self.s.PIPENV_VENV_IN_PROJECT or (
            ...
            and os.path.isdir(os.path.join(self.project_directory, ".flask"))
        )
    ...
    def get_location_for_virtualenv(self) -> str:
        ...
        if not self.project_directory:
            if self.is_venv_in_project():
                return os.path.abspath(".flask")
            return str(get_workon_home().joinpath(self.virtualenv_name))

        dot_venv = os.path.join(self.project_directory, ".flask")

        # If there's no .flask in project root, set location based on config.
        if not os.path.exists(dot_venv):
            if self.is_venv_in_project():
                return dot_venv
            return str(get_workon_home().joinpath(self.virtualenv_name))

        # If .flask in project root is a directory, use it.
        if os.path.isdir(dot_venv):
            return dot_venv

        # Now we assume .flask in project root is a file. Use its content.
        with open(dot_venv) as f:
            name = f.read().strip()

        # If .flask file is empty, set location based on config.
        if not name:
            return str(get_workon_home().joinpath(self.virtualenv_name))
